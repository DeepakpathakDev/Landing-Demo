...
router.get('/stockedge/insider-deals', async (req, res) => {
    try {
        const { exchange = 'BSE', page = 1, pageSize = 19, transactionType, dealMode } = req.query;
        ...
        const formattedData = {
            ...
            deals: response.data.map(deal => ({
                ...
                transitionDate: deal.TransitionDate,
                processDate: deal.ProcessDate,
                securityLogoUrl: deal.SecurityLogoUrl || `https://logo.clearbit.com/${deal.SecurityName.toLowerCase().replace(/\s+/g, '')}.com`
            }))
        };
...
router.get('/stockedge/search', async (req, res) => {
    try {
        ...
        const response = await axios.get(`https://api.stockedge.com/Api/SearchApi/GetSearchResults?searchText=${encodeURIComponent(query)}&page=1&pageSize=19&lang=en`, {
            headers: {
                'Accept': 'application/json',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
...
router.get('/stockedge/corporate-announcements', async (req, res) => {
    try {
        ...
        const pageSize = parseInt(req.query.pageSize) || 19;
...
// Get StockEdge intraday prices
router.get('/stockedge/intraday-prices/:listingId', async (req, res) => {
    try {
        const { listingId } = req.params;
        const { date = new Date().toISOString().split('T')[0] } = req.query;
        
        console.log(`Request received for intraday prices:`, { listingId, date });
        
        const data = await stockedgeService.fetchIntradayPrices(listingId, date);
        res.json(data);
    } catch (error) {
        console.error('Error in intraday prices route:', error);
        res.status(500).json({ 
...
// Get StockEdge news items
router.get('/stockedge/news', async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const pageSize = parseInt(req.query.pageSize) || 19;
        
        console.log(`Request received for news items:`, { page, pageSize });
        
        const cacheKey = `stockedge-news-${page}-${pageSize}`;
        const cachedData = cache.get(cacheKey);
        ...
        const response = await axios.get(`https://api.stockedge.com/Api/DailyDashboardApi/GetLatestNewsItems?page=${page}&pageSize=${pageSize}&lang=en`, {
            headers: {
                'Accept': 'application/json',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
        ...
        const formattedData = {
            data: response.data.map(item => ({
                ...
                securities: item.NewsitemSecurities,
                sectors: item.NewsitemSectors,
                industries: item.NewsitemIndustries
            })),
...
// Get StockEdge corporate action results
router.get('/stockedge/corporate-results', async (req, res) => {
    try {
        const page = parseInt(req.query.page) || 1;
        const pageSize = parseInt(req.query.pageSize) || 19;
        
        console.log(`Request received for corporate results:`, { page, pageSize });
        
        const cacheKey = `stockedge-corporate-results-${page}-${pageSize}`;
        const cachedData = cache.get(cacheKey);
        ...
        const response = await axios.get(`https://api.stockedge.com/Api/DailyDashboardApi/GetForthcomingCorporateActionResults?page=${page}&pageSize=${pageSize}&lang=en`, {
            headers: {
                'Accept': 'application/json',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
        ...
        console.error('Error fetching corporate results:', error);
        res.status(500).json({ 
